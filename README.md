...
During the testing phase, we ensured that the companies that were tested were never seen before by the model. Hence allowing the model to learn to expand its pattern recognition to entirely new companies. Thus these models could be utilized in a completely new company and will still perform very accurately with a very low error. In addition, the training set and validation set, both had unique companies to ensure the ability of this model to later be expanded onto entirely new companies. 
